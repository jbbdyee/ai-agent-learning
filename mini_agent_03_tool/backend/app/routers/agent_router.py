...
TAG_01_LLM = "01. LLM 기초"
TAG_02_STRUCTURED_OUTPUT = "02. Structured Output"
TAG_03_TOOL_USE = "03. Tool Use"

agent_router = APIRouter()


@agent_router.get("/health", tags=[TAG_01_LLM])
def health() -> dict:
    return {"status": "ok", "stage": "mini_agent_03_tool", "default_provider": settings.llm_provider}


@agent_router.get("/api/providers", tags=[TAG_01_LLM])
def providers() -> dict:
    return {"default_provider": settings.llm_provider, "providers": provider_status()}


@agent_router.post("/api/concepts/compare", response_model=ConceptCompareResult, tags=[TAG_01_LLM])
...
@agent_router.post("/api/travel/classify", response_model=TravelIntentResult, tags=[TAG_01_LLM])
...
@agent_router.post("/api/generate", response_model=GenerateResult, tags=[TAG_01_LLM])
...
@agent_router.post("/api/providers/compare", response_model=ProviderCompareResult, tags=[TAG_01_LLM])
...
@agent_router.post("/api/prompts/preview", response_model=PromptPreviewResult, tags=[TAG_02_STRUCTURED_OUTPUT])
...
@agent_router.post("/api/structured/validate", response_model=TravelValidationResult, tags=[TAG_02_STRUCTURED_OUTPUT])
...
@agent_router.post("/api/structured/travel-plan", response_model=StructuredTravelResult, tags=[TAG_02_STRUCTURED_OUTPUT])
...
@agent_router.post("/api/structured/compare", response_model=StructuredCompareResult, tags=[TAG_02_STRUCTURED_OUTPUT])
...
@agent_router.post("/api/media/image-analysis", tags=[TAG_01_LLM])
...
@agent_router.post("/api/media/tts", tags=[TAG_01_LLM])
...
@agent_router.get("/api/tools", tags=[TAG_03_TOOL_USE])
def tools() -> dict:
    return {"tools": get_tool_definitions(), "note": "모든 Tool은 조회 전용이며 예약이나 결제를 실행하지 않습니다."}


@agent_router.post("/api/tools/select", response_model=ToolSelectionResult, tags=[TAG_03_TOOL_USE])
...
@agent_router.post("/api/tools/compare", response_model=ToolCompareResult, tags=[TAG_03_TOOL_USE])
...
@agent_router.post("/api/tools/run", response_model=ToolRunResult, tags=[TAG_03_TOOL_USE])
...
@agent_router.post("/api/tools/complete", response_model=ToolCompleteResult, tags=[TAG_03_TOOL_USE])
